<template>
  <main>
    ...
    <div class="items-center mt-12 px-24" v-if="resultData && isDataLoaded">
      <h1 class="text-xl font-light mb-6 text-center">You searched for {{ result }}</h1>
      <div class="flex items-center" v-if="resultType == 'Professor'">
        <div class="flex-1 pr-8">
          ...
          <div class="flex justify-between mb-1">
            ...
            <span class="text-sm font-medium text-blue-700" v-if="resultData[1]">{{ resultData[1].avgDifficulty / 5.0 * 100 + '%' }}</span>
          </div>
          <div class="w-full bg-gray-200 rounded-full h-2.5 dark:bg-gray-700 mb-3" v-if="resultData[0]">
            <div class="bg-blue-600 h-2.5 rounded-full" :style="{ width: resultData[1].avgDifficulty / 5.0 * 100 + '%' }"></div>
          </div>
          <div v-else>
            No data available
          </div>
          <div class="flex justify-between mb-1">
            <span class="text-base font-medium text-blue-700">Rate my professor rating</span>
            <span class="text-sm font-medium text-blue-700" v-if="resultData[1]">{{ resultData[1].avgRating / 5.0 * 100 + '%' }}</span>
          </div>
          <div class="w-full bg-gray-200 rounded-full h-2.5 dark:bg-gray-700 mb-3" v-if="resultData[0]">
            <div class="bg-blue-600 h-2.5 rounded-full" :style="{ width: resultData[1].avgRating / 5.0 * 100 + '%' }"></div>
          </div>
          <div v-else>
            No data available
          </div>
          <div class="flex justify-between mb-1">
            ...
            <span class="text-sm font-medium text-blue-700" v-if="resultData[1]">{{ resultData[1].wouldTakeAgainPercent + '%' }}</span>
          </div>
          <div class="w-full bg-gray-200 rounded-full h-2.5 dark:bg-gray-700" v-if="resultData[0]">
            <div class="bg-blue-600 h-2.5 rounded-full" :style="{ width: resultData[1].wouldTakeAgainPercent + '%' }"></div>
          </div>
          <div v-else>
            No data available
          </div>
        ...
        <div class="flex-1 pl-8">
          ...
          <div class="flex justify-between mb-1">
            ...
            <span class="text-base font-medium text-blue-700" v-if="resultData[1]">{{ resultData[1].numRatings }} ratings</span>
          </div>
        </div>
      </div>
    </div>
