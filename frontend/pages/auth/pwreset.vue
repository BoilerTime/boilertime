<!--pwreset.vue is a page dedicated to allowing users to begin the password reset process.
 ...
 */
    async function sendemail() {
        // check if email has an account, if not: alert
        if (email.value==="power104@purdue.edu") {
            alert("This email has not been regeistered.")
        } else {
            // send the email
        }
    }
