<template>
  <main>
    <div v-if="friend_id == undefined && !isMobile">
      <NavBar />
    </div>
    <div v-else-if="friend_id == undefined && isMobile">
      <NavBarMobile />
    </div>
    <div
      id="capture"
      class="flex justify-center md:items-stretch h-screen md:h-fit p-3 bg-gray-200 dark:bg-neutral-500"
    >
      <div class="p-12" v-if="isDataLoaded">
        <LazyClassList
          v-for="course in scheduleData"
          :key="course.name"
          :data="course"
        />
      </div>
      ...
      <div v-if="!isMobile">
        <div id="calendar" v-if="result.length > 0">
          <button
            class="rounded-lg bg-yellow-500 hover:bg-yellow-700 px-4 py-2 text-sm font-bold border dark:border-black text-white"
            @click="screenie"
          >
            <svg
...
async function convertSchedule(schedule, blocks) {
  ...
  for (const course of schedule) {
    for (const meeting of course.meetings) {
      ...
      const easternEndTime = easternEndTimeDateTime.toLocaleTimeString(
        "en-US",
        { timeZone: "America/New_York", hour12: false }
      );
      const daysOfWeek = meeting.daysOfWeek.map((day) =>
        [
          "Sunday",
          "Monday",
          "Tuesday",
      ...
      async function getgpa(prof_name, class_name) {
        const response = await axios.post(
          "https://api.boilerti.me/api/getgpa",
          {
            prof_name: prof_name,
  ...
  for (const block of blocks) {
    ...
    const daysOfWeek = block.days_of_week.map((day) =>
      [
        "Sunday",
        "Monday",
        "Tuesday",
    ...
    const startDateTime = new Date(
      ...
      todayDate.getMonth(),
      todayDate.getDay(),
      block.start_time.substring(0, 2),
      block.start_time.substring(2, 4)
    );
    ...
    const easternEndTimeDateTime = new Date(
      startDateTime.getTime() + block.duration * 60 * 1000
    );
...
onBeforeMount(async () => {
  ...
  if (friend_id != undefined) {
    await axios
      .post(
        "https://api.boilerti.me/api/get/term/optimizedschedule",
        {
          user_id: friend_id,
...
function showWarning(configured) {
  if (configured) {
    $toast.info(
      "We try to fit your preferences, but sometimes it's difficult to find a schedule that satisfies all of them. ",
      {
        timeout: 5000,
      }
    );
