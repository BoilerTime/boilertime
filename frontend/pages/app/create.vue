<template>
  <main class="">
    ...
    <section
      class="flex flex-col h-screen overflow-auto p-10 bg-gray-100 dark:bg-neutral-600"
    >
      <div class="grid grid-row-5 md:grid-cols-5 md:gap-x-20">
        <div class="col-span-2">
          <div class="text-4xl font-bold text-black dark:text-gray-200 text-center md:text-start">
            Get started with
          </div>
          <div class="text-4xl font-bold text-yellow-500 text-center md:text-start">
            building your schedule
          </div>
          <div class="relative mt-8">
            <label class="font-semibold text-lg dark:text-gray-200"
              >📈 Trending classes:</label
            >
            <draggable
              v-model="trending_classes"
              group="classes"
              item-key="id"
            ...
            >
              <template #item="{ element }">
                <div
                  class="text-sm font-bold border dark:border-black p-1.5 bg-indigo-500 text-white rounded-md mr-3 hover:bg-red-500"
                >
                  {{ element }}
                </div>
              </template>
          ...
          <div class="relative mt-8">
            <label class="font-semibold text-lg dark:text-gray-200"
              >⭐ Classes usually taken together with {{ lastEntered }}:</label
            >
            <draggable
              v-model="together_classes"
              group="classes"
              item-key="id"
              class="relative flex p-3 mt-3 border-2 border-gray-300 border-dashed rounded-lg"
            >
        ...
        <div
          class="p-8 my-12 bg-white rounded-lg shadow-2xl dark:bg-neutral-700 col-span-3"
        >
          <div class="relative">
            <div class="mb-8"></div>
            <label class="font-semibold text-md dark:text-gray-200"
              >Add classes you have to take:</label
            >
            <input
              v-model="searchTerm"
            ...
            >
              ...
              >
                <span
                  class="flex items-center bg-yellow-500"
                  v-if="bookmarked_classes.includes(result)"
                >
            ...
            <draggable
              v-model="selectedRequiredCourses"
              group="classes"
              item-key="id"
            ...
            >
              <template #item="{ element, index }">
                <div
                  class="text-sm font-bold border dark:border-black p-1.5 bg-indigo-500 text-white rounded-md mr-3 mt-3 hover:bg-red-500"
            ...
            <div class="relative mt-8">
              <label class="font-semibold text-md dark:text-gray-200"
                >Add classes you want to take:</label
              >
              <input
                v-model="optionalSearchTerm"
                class="w-full px-4 py-2 mt-3 border rounded-md shadow-sm focus:outline-none focus:ring-2 dark:bg-neutral-500 dark:placeholder-neutral-600 focus:ring-indigo-500 focus:border-indigo-500 dark:border-black"
                placeholder="Search for classes..."
                @keyup.enter="addSingleOptionalToSelected"
              />
              <ul
                v-if="
                  isOptionalSearchActive && filteredOptionalResults.length > 0
                "
                class="absolute z-10 w-full mt-1 overflow-scroll bg-white rounded-lg shadow-lg dark:bg-neutral-600 dark:text-gray-200 outline-black max-h-48"
              >
                <li
              ...
              <draggable
                v-model="selectedOptionalCourses"
                group="classes"
                item-key="id"
                class="flex flex-wrap"
              >
            ...
            <div class="relative mt-8">
              <label class="font-semibold text-md dark:text-gray-200"
                >Drag classes here to bookmark them for later:</label
              >
              <draggable
                v-model="bookmarked_classes"
                group="classes"
                item-key="id"
                class="relative flex p-3 mt-3 border-2 border-gray-300 border-dashed rounded-lg"
              >
            ...
            <div class="flex md:gap-4 md:justify-end mt-8">
              <button
                @click="showPrefMenu()"
                class="p-2 font-bold text-white bg-indigo-500 border hover:bg-indigo-700 text-md dark:border-black rounded-md"
              >
                Customize Preferences
              </button>
              <button
                @click="blockConfig = true"
                class="p-2 font-bold text-white bg-indigo-500 border hover:bg-indigo-700 text-md dark:border-black rounded-md"
              >
                Block Time
              </button>
              <button
                @click="submit"
                class="p-2 font-bold text-white bg-yellow-500 border hover:bg-yellow-700 text-md dark:border-black rounded-md"
              >
                Submit
              </button>
          ...
          >
            ...
            >
              ...
              <div class="mt-2">
                <p
                  v-if="!inLine"
                  class="text-sm text-center text-gray-500 dark:text-gray-200"
                >
                ...
                <div v-if="multiLoader" class="items-center justify-center">
                  <div class="flex items-center justify-center">
                    <div
                      class="flex items-center justify-center rounded-full h-28 w-28 bg-gradient-to-tr from-yellow-500 to-gray-500 animate-spin"
                    >
                      <div
                        class="w-20 h-20 bg-white rounded-full dark:bg-neutral-700"
                      ></div>
                    </div>
                  </div>
                </div>
              ...
              <button
                @click="displayTips = true"
                class="float-right p-2 font-bold text-white bg-yellow-500 border hover:bg-yellow-700 text-md dark:border-black rounded-md"
            ...
            >
              ...
              <div class="mt-2">
                ...
                <p class="text-xs text-gray-500">
                  <i
                    >Note, becuase optimization relies on ML, some options may
                    not look correct.
                  </i>
                </p>
              </div>
    ...
    <Dialog as="div" class="relative z-10">
      ...
      <div class="fixed inset-0 overflow-y-auto">
        ...
        >
          ...
          >
            <DialogPanel
              class="w-full max-w-md p-6 overflow-hidden text-left align-middle bg-white dark:bg-neutral-500 shadow-xl transform rounded-2xl transition-all"
            >
              <DialogTitle
                as="h1"
                class="text-3xl font-bold text-center text-gray-900 dark:text-white leading-6"
              >
                Your Schedule <span class="text-yellow-500">Preferences</span>
              </DialogTitle>
              <div class="mt-2">
                <p class="text-md text-center text-gray-500 dark:text-white">
                  Let's make a schedule that
                  <span class="text-yellow-500">works for you</span>
                </p>
                <!-- Data items -->
                <br />
                <label class="font-semibold text-md dark:text-gray-200"
                  >Select your preference order:</label
                >
                ...
                <br />
                <label class="font-semibold text-md dark:text-gray-200"
                  >Select your time of day preference:</label
                >
                <fieldset class="mt-2">
                  <div
                  ...
                  >
                    ...
                    >
                      ...
                      <label
                        :for="time.id"
                        class="block ml-3 text-sm font-medium text-gray-900 leading-6 dark:text-gray-200"
                        >{{ time.title }}</label
                      >
                    </div>
                  </div>
              ...
              <br />
              <label class="font-semibold text-md dark:text-gray-200"
                >How many classes do you want to take?</label
              >
              <div>
                <input
                  class="shadow appearance-none border border-yellow-500 rounded w-half py-2 px-3 text-gray-700 mb-3 leading-tight focus:outline-none focus:shadow-outline"
                  v-model="courseCount"
                  placeholder="Between 1 and 5 Courses"
                  @input="validateInput()"
                />
              </div>
              <div class="mt-2">
                <p class="text-xs text-gray-500">
                  <i
                    >Note, because optimization relies on Artificial
                    Inteligence, we can't guarentee your preferences will be
                    honored
                  </i>
                </p>
              </div>
              <button
                ...
                class="float-right bg-yellow-500 hover:bg-yellow-700 text-white p-2 text-md font-bold border dark:border-black rounded-md"
                style="align: text-right"
              >
                Save
              </button>
        ...
        >
          ...
          >
            ...
            >
              <DialogTitle
                as="h1"
                class="text-3xl font-bold text-center text-gray-900 dark:text-white leading-6 pb-8"
              >
                Your <span class="text-yellow-500">Blocked Times</span>
              </DialogTitle>
              <div class="bg-white dark:bg-neutral-700 shadow-md rounded px-8 pt-6 pb-8 mb-4">
                <p class="text-sm text-gray-500 dark:text-white text-left">
                  Have a meeting? Need some time off? Let's find times that
                  <span class="text-yellow-500">work for you</span>
                </p>
              </div>
              <div
                v-for="(entry, index) in blockArray"
                style="transition: width 2s"
              >
                <div class="bg-white dark:bg-neutral-700 shadow-md rounded px-8 pt-6 pb-8 mb-4">
                  <label class="dark:text-white"
                    ><b>Block {{ index + 1 }}: {{ entry.name }}</b>
                    <button
                      type="button"
                      class="float-right text-red-700 border border-red-700 hover:bg-red-700 hover:text-white focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm p-2.5 text-center inline-flex items-center mr-2 dark:border-blue-500 dark:text-blue-500 dark:hover:text-white dark:focus:ring-blue-800 dark:hover:bg-blue-500"
                      @click="deleteBlock(index)"
                    >
                      <TrashIcon class="w-4 h-4" /><span class="sr-only"
                        >Delete</span
                      >
                    </button>
                  </label>
                  <p class="dark:text-white">Start Time: {{ entry.start }}</p>
                  <p class="dark:text-white">Duration: {{ entry.duration }}</p>

                  <br />
                  <div class="flex items-left justify-left">
                    <div
                      class="grid grid-cols-5 grid-rows-1 gap-4 rounded-xl border-4"
                    >
                      <div>
                        <input
                          type="radio"
                          :id="'Monday' + index"
                          class="peer hidden"
                          :checked="entry.daysOfWeek[0] == true"
                          v-on:click="
                            entry.daysOfWeek[0] = !entry.daysOfWeek[0]
                          "
                        />
                        <label
                          :for="'Monday' + index"
                          class="col-start-4 block cursor-pointer select-none rounded-xl p-2 text-center peer-checked:bg-blue-500 peer-checked:font-bold peer-checked:text-white"
                          >M</label
                        >
                      </div>
                      <div>
                        <input
                          ...
                          :id="'Tuesday' + index"
                          class="peer hidden"
                          :checked="entry.daysOfWeek[1] == true"
                          v-on:click="resetArray(index, 1)"
                        />
                        <label
                          :for="'Tuesday' + index"
                          class="col-start-4 block cursor-pointer select-none rounded-xl p-2 text-center peer-checked:bg-blue-500 peer-checked:font-bold peer-checked:text-white"
                      ...
                      <div>
                        <input
                          ...
                          :id="'Wednesday' + index"
                          class="peer hidden"
                          :checked="entry.daysOfWeek[2] == true"
                          v-on:click="resetArray(index, 2)"
                        />
                        <label
                          :for="'Wednesday' + index"
                          class="col-start-4 block cursor-pointer select-none rounded-xl p-2 text-center peer-checked:bg-blue-500 peer-checked:font-bold peer-checked:text-white"
                      ...
                      <div>
                        <input
                          ...
                          :id="'Thursday' + index"
                          class="peer hidden"
                          :checked="entry.daysOfWeek[3] == true"
                          v-on:click="resetArray(index, 3)"
                        />
                        <label
                          :for="'Thursday' + index"
                          class="col-start-4 block cursor-pointer select-none rounded-xl p-2 text-center peer-checked:bg-blue-500 peer-checked:font-bold peer-checked:text-white"
                      ...
                      <div>
                        <input
                          ...
                          :id="'Friday' + index"
                          class="peer hidden"
                          :checked="entry.daysOfWeek[4] == true"
                          v-on:click="resetArray(index, 4)"
                        />
                        <label
                          :for="'Friday' + index"
                          class="col-start-4 block cursor-pointer select-none rounded-xl p-2 text-center peer-checked:bg-blue-500 peer-checked:font-bold peer-checked:text-white"
              ...
              <div class="mb-4">
                <button
                  @click="showSectionConfig()"
                  class="w-full text-indigo-100 transition-colors duration-150 bg-indigo-700 rounded-lg focus:shadow-outline hover:bg-indigo-800"
                >
                  {{ blockOption }}
                </button>
                <br />
              ...
              <div v-if="isAddingBlock">
                ...
                >
                  ...
                  <div>
                    ...
                    >
                      <div
                        class="grid grid-cols-5 grid-rows-1 gap-4 bg-gray-200 rounded-xl"
                      >
                        <div v-for="(days, index) in daysPref">
                          <input
                            ...
                            :id="days"
                            class="peer hidden"
                            :checked="dayPrefActive[index]"
                            v-on:click="
                              dayPrefActive[index] = !dayPrefActive[index]
                            "
                          />
                          <label
                            :for="days"
            ...
            >
              ...
              <div>
                ...
                <ul class="text-sm list-disc list-inside">
                  <li>
                    We use an advanced algorithm that uses data like coure times
                    and RMP ratings then combine it with your preferences
                  </li>
                  <li>
                    After generating literally thousands of options, we select
                    the best couple of options and make those your schedules
                  </li>
                  <li>
                    Becuase the algorithm uses a lot of processing power, we can
                    only let a limited number of devices use it at once
                  </li>
                </ul>
              </div>
              <br />
              <div>
                <p class="text-xl">How to Use It</p>
                <ul class="text-sm list-disc list-inside">
                  <li>
                    It looks like you're already an expert! Congrats on making a
                    schedule
                  </li>
                  <li>
                    Next time, look at our hub first to discover classes you
                    might want to take and see a bit of info first
                    <ul
                      class="text-sm list-none list-inside"
                      style="margin-left: 1%"
                    >
                      <li>
                        - That way, you can see if the right professors, times
                        of day, or locations are there for you
                      </li>
                      <li>
                        - You can always bookmark a course to come back later if
                  ...
                  >
                    <li>
                      - To save everyone time and money, take a look over the
                      class options before optimizing
                    </li>
                    <li>
                      - Never re-optimize the same schedule unless you don't
                      like any of the results
                    </li>
                    <li>
                      - If you've inputted optional classes that don't show up
                      in any of the options, remove another class before
                      re-optimizing
                    </li>
                  </ul>
                  <li>
                    Be realistic about the number of classes you'd like to take,
                    the number of classes inputted will be filled
                  </li>
                </ul>
              </div>
              <button
                @click="closeTips()"
                class="p-2 font-bold text-white bg-yellow-500 border float-middle hover:bg-red-700 text-md dark:border-black rounded-md"
                style="align: text-center"
              >
                Close
              </button>
...
const filteredOptionalResults = computed(() => {
  ...
  return optionalData.value.filter((item) => {
    return item
      .toLowerCase()
      .startsWith(optionalSearchTerm.value.toLowerCase());
  });
});
...
function removeFromBookmarked(index) {
  ...
  axios
    .post(
      "https://api.boilerti.me/api/removebookmark",
      {
        user_id: userStore.user_id,
...
watch(bookmarked_classes, (newVal, oldVal) => {
  if (newVal.length > oldVal.length) {
    ...
    if (!isAGuest.value) {
      ...
      axios
        .post(
          "https://api.boilerti.me/api/addbookmark",
          {
            user_id: userStore.user_id,
            class_name: bookmarked_classes.value,
          },
...
function submit() {
  ...
  if (getNumCourses() < selectedRequiredCourses.value.length) {
    $toast.error(
      "Your settings only contain " +
        getNumCourses() +
        " course" +
        pluralize(getNumCourses()) +
        ", but you've inputted " +
        selectedRequiredCourses.value.length +
        " required course" +
        pluralize(selectedRequiredCourses.value.length),
      {
        timeout: 5000,
      }
    );
  ...
  if (
    getNumCourses() >
    selectedRequiredCourses.value.length + selectedOptionalCourses.value.length
  ) {
    $toast.warning(
      "Your settings contain " +
        ...
        ", but you've inputted " +
        (selectedRequiredCourses.value.length +
          selectedOptionalCourses.value.length) +
        " course" +
        pluralize(
          selectedRequiredCourses.value.length +
            selectedOptionalCourses.value.length
        ) +
        ". We'll round down for you.",
      {
        timeout: 5000,
      }
    );
  ...
  if (selectedRequiredCourses.value.length > 0) {
    ...
    axios
      .post(
        "https://api.boilerti.me/api/createschedule",
        {
          user_id: userStore.user_id,
...
function saveOptimizedSchedule(schedule) {
  axios
    .post("https://api.boilerti.me/api/saveoptimizedschedule", {
      data: schedule,
      user_id: userStore.user_id,
    })
    .then(() => {
...
function inQueue(position, size) {
  ...
  if (!inLine.value) status.value = messages[randInt(messages.length - 1)]; //"Getting Course Data"
  inLine.value = true;
  algorithmProgress.value = false;
...
function saveScheduleToDB() {
  ...
  if (!isAGuest.value) {
    ...
    axios
      .post("https://api.boilerti.me/api/saveschedule/guest", {
        user_id: userStore.user_id,
        required_classes: selectedRequiredCourses.value,
        optional_classes: selectedOptionalCourses.value,
        time: getTimePref(),
...
function configureBlocks(blockData) {
  ...
  for (let i = 0; i < blockData.length; i++) {
    ...
    for (let j = 0; j < daysOfWeekFull.length; j++) {
      console.log(
        blockData[i].days_of_week.filter((day) => day == daysOfWeekFull[j])
      );
      if (
        blockData[i].days_of_week.filter((day) => day == daysOfWeekFull[j])
          .length > 0
